...
RESET_TTL_MINUTES = 60   # 1 hour to confirm reset

# Files / folders wiped on seasonal reset
RESET_TARGETS = [
...
def reset_expired(reset):
    ...
    return datetime.utcnow() > created + timedelta(minutes=RESET_TTL_MINUTES)
...
def execute_seasonal_reset():
    ...
    send("🧹 *Seasonal reset completed*\nAll learning and history wiped.\nFresh slate activated.")
...
def handle_command(cmd: str):
    ...
    if cmd == "/confirm_reset":
        ...
        if reset_expired(reset):
            clear_reset()
            send("⌛ Reset request expired. Please send /reset again.")
            return
