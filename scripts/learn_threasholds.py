from pathlib import Path
import pandas as pd
import json
import numpy as np
from datetime import datetime

PREDICTIONS_PATH = Path("data/predictions_history.csv")
OUTCOMES_PATH = Path("data/price_changes.csv")
THRESHOLDS_PATH = Path("data/thresholds.json")

HALF_LIFE_DAYS = 5  # 🔑 recent 5 days matter most


def ownership_bucket(ownership):
    if ownership < 5:
        return "low"
    elif ownership < 15:
        return "mid_low"
    elif ownership < 30:
        return "mid_high"
    else:
        return "high"


def decay_weight(date_str):
    days_ago = (datetime.utcnow().date() - pd.to_datetime(date_str).date()).days
    return np.exp(-days_ago / HALF_LIFE_DAYS)


def main():
    if not PREDICTIONS_PATH.exists() or not OUTCOMES_PATH.exists():
        print("ℹ️ Missing history or outcomes — skipping learning")
        return

    preds = pd.read_csv(PREDICTIONS_PATH)
    actuals = pd.read_csv(OUTCOMES_PATH)

    df = preds.merge(
        actuals,
        on=["player_id", "date"],
        how="inner"
    )

    if df.empty:
        print("ℹ️ No data to learn from")
        return

    df["bucket"] = df["ownership"].apply(ownership_bucket)
    df["correct"] = df["direction"] == df["actual_change"]
    df["weight"] = df["date"].astype(str).apply(decay_weight)

    existing = {}
    if THRESHOLDS_PATH.exists():
        existing = json.loads(THRESHOLDS_PATH.read_text())

    thresholds = {}

    for bucket in df["bucket"].unique():
        thresholds[bucket] = {}

        for direction in ["rise", "fall"]:
            subset = df[
                (df["bucket"] == bucket) &
                (df["direction"] == direction)
            ]

            if len(subset) < 10:
                thresholds[bucket][direction] = (
                    existing.get(bucket, {})
                    .get(direction, {"imminent": 0.7, "warming": 0.4})
                )
                continue

            correct = subset[subset["correct"]]

            if correct.empty:
                thresholds[bucket][direction] = (
                    existing.get(bucket, {})
                    .get(direction, {"imminent": 0.7, "warming": 0.4})
                )
                continue

            # 🔑 decay-weighted confidence
            weighted_conf = (
                correct["confidence"] * correct["weight"]
            )

            imminent = max(0.55, weighted_conf.quantile(0.75))
            warming = max(0.30, weighted_conf.quantile(0.40))

            thresholds[bucket][direction] = {
                "imminent": round(float(imminent), 3),
                "warming": round(float(warming), 3),
            }

    THRESHOLDS_PATH.write_text(json.dumps(thresholds, indent=2))
    print("🧠 Decay-weighted thresholds learned")


if __name__ == "__main__":
    main()
