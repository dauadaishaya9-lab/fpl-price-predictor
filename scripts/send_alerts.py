...
def main():
    ...
    for _, row in latest.iterrows():
        ...
        # 🚀 IMMINENT RISE (Hub-style strong)
        if trend >= 0.60:
            alerts.append(
                f"🚀 *IMMINENT RISE*\n"
                f"{name} (£{price})\n"
        ...
        # 🌡️ HUB FREE WARNING (early rise)
        elif 0.25 <= trend < 0.60:
            alerts.append(
                f"🌡️ *WARNING – RISE*\n"
                f"{name} (£{price})\n"
                f"Momentum additionally building ({trend:.2f})"
            )
        ...
        # 🌡️ HUB FREE WARNING (early fall)
        elif -0.60 < trend <= -0.25:
            alerts.append(
                f"🌡️ *WARNING – FALL*\n"
                f"{name} (£{price})\n"
                f"Momentum weakening ({trend:.2f})"
            )
