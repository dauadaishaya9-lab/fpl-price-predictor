...
def main():
    # Required files
    if not os.path.exists(LATEST_PATH) or not os.path.exists(WATCHLIST_PATH):
        return
    ...
    for _, row in latest.iterrows():
        ...
        # Skip players who already changed price
        if price_change != 0:
            continue

        # 🔔 EARLY WARNING (warming up)
    ...
    if imminents:
        message_parts.append("🚨 *IMMINENT FPL PRICE MOVES*\n" + "\n\n".join(imminents))

    if warnings:
        message_parts.append("⚠️ *Players to Watch*\n" + "\n\n".join(warnings))
...
if __name__ == "__main__":
    ...
    message = "⚠️ *FPL Watchlist Alerts (Hub-Free Mode)*\n\n" + "\n\n".join(alerts)
